...
// TestCoreDeps_ApplyDefaults_AllNilFieldsGetDefaults verifies that nil dep fields are populated.
func TestCoreDeps_ApplyDefaults_AllNilFieldsGetDefaults(t *testing.T) {
	d := &CoreDeps{}
	d.applyDefaults()

	assert.NotNil(t, d.TLSCertEnsurer, "TLSCertEnsurer should have default")
	assert.NotNil(t, d.ControlTLSLoader, "ControlTLSLoader should have default")
	assert.NotNil(t, d.ControlServerFactory, "ControlServerFactory should have default")
	assert.NotNil(t, d.ObservabilityServerFactory, "ObservabilityServerFactory should have default")
	assert.NotNil(t, d.CertsDirGetter, "CertsDirGetter should have default")
	assert.NotNil(t, d.DatabaseURLGetter, "DatabaseURLGetter should have default")
	assert.NotNil(t, d.MigratorFactory, "MigratorFactory should have default")
	assert.NotNil(t, d.AutoMigrateGetter, "AutoMigrateGetter should have default")
}

// TestCoreDeps_ApplyDefaults_NonNilFieldsPreserved verifies that existing funcs are not overwritten.
func TestCoreDeps_ApplyDefaults_NonNilFieldsPreserved(t *testing.T) {
	sentinel := "called"
	customCalled := false
	...
	// Custom DatabaseURLGetter must still be the one we supplied.
	result := d.DatabaseURLGetter()
	assert.True(t, customCalled, "custom DatabaseURLGetter was replaced")
	assert.Equal(t, sentinel, result)

	// Custom AutoMigrateGetter returns false, not the default parseAutoMigrate.
	assert.False(t, d.AutoMigrateGetter())
}

// TestCoreDeps_ApplyDefaults_Idempotent verifies that calling applyDefaults twice is safe.
func TestCoreDeps_ApplyDefaults_Idempotent(t *testing.T) {
	d := &CoreDeps{}
	d.applyDefaults()
	first := d.DatabaseURLGetter

	d.applyDefaults()
	// The function pointers may differ (closures recreated), but the behaviour must match.
	require.NotNil(t, d.DatabaseURLGetter)
	require.NotNil(t, first)
}

// TestCoreDeps_ApplyDefaults_DatabaseURLGetterReadsEnv verifies the default reads DATABASE_URL.
func TestCoreDeps_ApplyDefaults_DatabaseURLGetterReadsEnv(t *testing.T) {
	t.Setenv("DATABASE_URL", "postgres://test:test@localhost/testdb")

	d := &CoreDeps{}
	d.applyDefaults()

	assert.Equal(t, "postgres://test:test@localhost/testdb", d.DatabaseURLGetter())
}

// TestCoreDeps_ApplyDefaults_DatabaseURLGetterEmptyWhenUnset verifies empty string when env unset.
func TestCoreDeps_ApplyDefaults_DatabaseURLGetterEmptyWhenUnset(t *testing.T) {
	t.Setenv("DATABASE_URL", "")

	d := &CoreDeps{}
	d.applyDefaults()

	assert.Empty(t, d.DatabaseURLGetter())
}
