...
func VerifyAccountScoping(ctx context.Context, conn *nats.Conn) error {
	...
	// Each probe installs its OWN violation channel + async error handler (see
	// probeDenied). A shared channel across both probes could let a straggler
	// violation from the subscribe probe — dispatched on nats.go's async
	// callback goroutine after the subscribe probe finished but during the
	// publish probe's wait — be mis-read as the publish probe's result, a
	// false "denied" that would let a publish-over-scoped account boot
	// (fail-open). Isolating the channel/handler per probe keeps such a
	// straggler in the subscribe probe's now-dead channel.

	// Probe subscribe: a denied SUB raises a permissions violation.
	subDenied, err := probeDenied(ctx, conn, func() error {
	...
	pubDenied, err := probeDenied(ctx, conn, func() error {
...
//
// Each call installs its OWN violation channel + async error handler for just
// the duration of this single probe. A shared channel would let a violation
// generated by a PRIOR probe (e.g. the subscribe probe's async flush/teardown,
// dispatched later on nats.go's callback goroutine) be mis-attributed to this
// probe — a false "denied" that could let an over-scoped account boot
// (fail-open). The fresh channel eliminates that shared-buffer drain race for
// any straggler that arrives while the prior probe's handler is still installed:
// it lands in the prior probe's now-dead channel rather than this one.
//
// RESIDUAL (documented known-limitation, not fully closed here): the async
// error handler is a SINGLE connection-level slot, swapped between probes on the
// shared long-lived connection. Isolation is therefore timing-dependent, not
// structural. Once the subscribe probe returns and its defer restores the prior
// handler, the publish probe installs its own handler for its wait window; a
// SECOND subscribe-origin violation dispatched by nats.go's callback goroutine
// during that window would land on the publish probe's handler → publish channel
// → a false "denied" for the publish probe. That is a theoretical fail-open, but
// only on an asymmetric account (SUBSCRIBE to forbidden.> denied while PUBLISH is
// permitted — the publish probe only runs after subscribe was denied) AND only
// if a denied subscribe emits more than one violation (the flush round-trip
// normally emits exactly one; teardown Unsubscribe of an already-rejected sub
// typically emits none), so the window is very small. A structural cure — a
// dedicated short-lived *nats.Conn per probe, or a conn.Flush()+settle to drain
// the callback path before installing the next handler — is deferred: its
// fail-closed correctness can only be validated against a real broker configured
// with a subscribe-denied / publish-permitted account (Docker integration tier),
// so it is not attempted blind. Do NOT overclaim isolation this code does not
// provide.
//
// The handler restores whatever handler was installed before this probe:
// VerifyAccountScoping runs on the shared long-lived connection (event bus,
// audit projection, cluster, invalidation all share it), so leaving our probe
// closure installed would orphan any later component relying on connection-level
...
func probeDenied(
	...
	// Buffered so nats.go's callback goroutine never blocks on dispatch; the
	// subscribe probe can raise more than one violation (flush + teardown).
