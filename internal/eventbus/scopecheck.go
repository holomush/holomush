...
// (fail-open). With a fresh channel, such a straggler lands in the prior
// probe's now-dead channel rather than this one. The handler restores whatever
// handler was installed before this probe: VerifyAccountScoping runs on the
// shared long-lived connection (event bus, audit projection, cluster,
// invalidation all share it), so leaving our probe closure installed would
// orphan any later component relying on connection-level async error surfacing.
func probeDenied(
	ctx context.Context,
	conn *nats.Conn,
	op func() error,
