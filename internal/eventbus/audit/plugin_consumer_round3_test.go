...
// TestBuildAuditRowProjectionFromEnvelope is the happy path proving
// buildAuditRow extracts projection fields without decryption (Phase 7
// replacement for the deleted TestDecodeEnvelopeIdentityCodecDecodesSuccess).
func TestBuildAuditRowProjectionFromEnvelope(t *testing.T) {
	t.Parallel()
	h := validPluginHeaders(t)
	row, err := buildAuditRow(&stubMsg{
		headers: h,
		subject: "events.main.plugin.test",
		data:    newValidEnvelopeBytes(t),
	})
