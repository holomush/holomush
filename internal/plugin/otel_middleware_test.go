...
func TestServiceProxyMiddlewareQueryLocation(t *testing.T) {
	...
	mockProxy := mocks.NewMockServiceProxy(t)
	loc := &plugins.LocationResult{ID: "loc-1", Name: "The Void"}
	mockProxy.EXPECT().
		QueryLocation(mock.Anything, "subj", "loc-1").
		Return(loc, nil)

	mw, err := plugins.NewServiceProxyMiddleware(mockProxy, h.tracerProvider, h.meterProvider)
	require.NoError(t, err)

	got, err := mw.QueryLocation(context.Background(), "subj", "loc-1")
	require.NoError(t, err)
	assert.Equal(t, "The Void", got.Name)

	spans := h.spans()
	require.Len(t, spans, 1)
	assert.Equal(t, "plugin.service.query_location", spans[0].Name)

	rm := h.metrics(t)
	assertHistogramExists(t, rm, "plugin_service_duration_seconds")
...
func TestServiceProxyMiddlewareError(t *testing.T) {
	...
	mockProxy := mocks.NewMockServiceProxy(t)
	mockProxy.EXPECT().
		QueryCharacter(mock.Anything, "subj", "char-1").
		Return(nil, errors.New("not found"))

	mw, err := plugins.NewServiceProxyMiddleware(mockProxy, h.tracerProvider, h.meterProvider)
	require.NoError(t, err)

	_, err = mw.QueryCharacter(context.Background(), "subj", "char-1")
	require.Error(t, err)
...
func TestServiceProxyMiddlewareMultipleOperations(t *testing.T) {
	...
	mockProxy := mocks.NewMockServiceProxy(t)
	mockProxy.EXPECT().KVSet(mock.Anything, "p", "k", "v").Return(nil)
	mockProxy.EXPECT().EmitEvent(mock.Anything, "s", "say", []byte("{}")).Return(nil)
	mockProxy.EXPECT().Log(mock.Anything, "info", "test")

	mw, err := plugins.NewServiceProxyMiddleware(mockProxy, h.tracerProvider, h.meterProvider)
	require.NoError(t, err)

	require.NoError(t, mw.KVSet(context.Background(), "p", "k", "v"))
	require.NoError(t, mw.EmitEvent(context.Background(), "s", "say", []byte("{}")))
	mw.Log(context.Background(), "info", "test") // no span for Log

	spans := h.spans()
	assert.Len(t, spans, 2) // KVSet + EmitEvent; Log has no span

	rm := h.metrics(t)
	assertCounterValue(t, rm, "plugin_service_calls_total", 2)
}
