...
// The initial 18 (T22) plus 5 gap-fill policies (T22b: G1-G4), 2 phase-2 command policies,
// 2 system bootstrap policies, and 11 channel policies (Epic 10).
// The player-basic-commands seed (v4) also adds "channel" to the allowed command list.
...
func SeedPolicies() []SeedPolicy {
	return []SeedPolicy{
		...
		},
		{
			...
			DSLText:     `permit(principal is character, action in ["execute"], resource is command) when { resource.command.name in ["say", "pose", "look", "go", "quit", "page", "whisper", "emit", "who", "examine", "help", "alias", "unalias", "aliases", "set", "channel"] };`,
			SeedVersion: 4,
		},
		{
		...
		// Character-level policies for channel authorization. These are system seeds
		// (not plugin policies) because they govern character authorization, which
		// requires principal type "character". Plugin manifest policies are scoped
		// to principal type "plugin" only.
		//
		// Note: Channel-specific attribute resolution (resource.channel.*,
		// principal.character.channel_*) requires a ChannelAttributeProvider
		// (not yet implemented). Until then, the plugin handles its own authorization.
