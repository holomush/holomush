...
// Plugin returns a Writable over the named plugin's isolated game-scope
// partition. Every key is transparently prefixed with "plugin/<name>/" before
// it reaches holomush_system_info, so a plugin's keys can neither collide with
// nor be read by the host partition or any other plugin. Plugin keys are NOT
...
func (g *postgresGameSettings) Plugin(name string) Writable {
	return &gamePluginSettings{store: g.store, prefix: ReservedNamespace + "/" + name + "/"}
}

// gamePluginSettings is the plugin-partitioned Writable over
// holomush_system_info. It wraps the same SystemInfoStore as
// postgresGameSettings but prefixes every key with "plugin/<name>/" and skips
// namespace validation (the plugin owns its keyspace). Storage encoding matches
// the host game scope: scalars are stored as raw strings, slices as
// JSON-array-encoded strings.
type gamePluginSettings struct {
	store  SystemInfoStore
	prefix string
}
...
func (o *gamePluginSettings) readString(ctx context.Context, key string) (string, bool) {
	v, err := o.store.GetSystemInfo(ctx, o.prefix+key)
	if err != nil {
		if !errors.Is(err, ErrNotFound) {
			slog.DebugContext(ctx, "game plugin settings read failed",
...
func (o *gamePluginSettings) StringN(ctx context.Context, key string) (string, bool) {
	return o.readString(ctx, key)
}

func (o *gamePluginSettings) IntN(ctx context.Context, key string) (int, bool) {
	...
	v, err := strconv.Atoi(s)
	if err != nil {
		slog.DebugContext(ctx, "game plugin settings int parse failed",
			"key", key, "raw", s, "error", err)
		return 0, false
	}
	return v, true
...
func (o *gamePluginSettings) BoolN(ctx context.Context, key string) (value, ok bool) {
	s, ok := o.readString(ctx, key)
	if !ok {
		return false, false
	}
	v, err := strconv.ParseBool(s)
	if err != nil {
		slog.DebugContext(ctx, "game plugin settings bool parse failed",
			"key", key, "raw", s, "error", err)
		return false, false
	}
	return v, true
...
func (o *gamePluginSettings) DurationN(ctx context.Context, key string) (time.Duration, bool) {
	s, ok := o.readString(ctx, key)
	if !ok {
		return 0, false
	}
	v, err := time.ParseDuration(s)
	if err != nil {
		slog.DebugContext(ctx, "game plugin settings duration parse failed",
			"key", key, "raw", s, "error", err)
		return 0, false
	}
	return v, true
...
func (o *gamePluginSettings) StringSliceN(ctx context.Context, key string) ([]string, bool) {
	s, ok := o.readString(ctx, key)
	if !ok {
		return nil, false
	}
	var out []string
	if err := json.Unmarshal([]byte(s), &out); err != nil {
		slog.DebugContext(ctx, "game plugin settings string-slice parse failed",
			"key", key, "raw", s, "error", err)
		return nil, false
	}
	return out, true
...
func (o *gamePluginSettings) SetString(ctx context.Context, key, value string) error {
...
func (o *gamePluginSettings) SetStringSlice(ctx context.Context, key string, values []string) error {
	encoded, err := json.Marshal(values)
	if err != nil {
		return oops.With("key", key).With("prefix", o.prefix).Wrap(err)
	}
...
var (
	...
	_ Writable = (*gamePluginSettings)(nil)
)
