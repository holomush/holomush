// Isolated tool-dependency module for general build/test tooling
// (Go 1.24+ `go tool`).
//
// Kept separate from the main go.mod so the application's dependency graph
// stays lean, and split from go.tool-lint.mod because the linters' closures
// (golangci-lint's analyzers, actionlint's YAML parser) conflict with the
// charmbracelet/yaml versions task pulls in — a single shared tools module
// fails to compile.
//
// Tools are fetched from the Go module proxy (not GitHub Releases — removes
// the CDN-flake class that broke `arduino/setup-task` and the install.sh
// shim, holomush-06vz / holomush-ar31b) and run via
// `GOWORK=off go tool -modfile=go.tool.mod <name>` (see Taskfile.yaml GO_TOOL).
