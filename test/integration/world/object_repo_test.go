...
var _ = Describe("ObjectRepository", func() {
	...
	Describe("Create", func() {
		It("creates object with containment in location", func() {
			...
			err := delErr(env.Objects.Create(ctx, obj))
	...
	Describe("Move", func() {
		...
		It("prevents circular containment", func() {
			...
			err := delErr(env.Objects.Move(ctx, c1.ID, world.Containment{ObjectID: &c2.ID}, 0))
			Expect(err).To(HaveOccurred())
	...
	Describe("ListAtLocation", func() {
		It("returns objects at location", func() {
			Expect(delErr(env.Objects.Create(ctx, createTestObject("Obj1", "First.", world.Containment{LocationID: &room.ID})))).To(Succeed())
			Expect(delErr(env.Objects.Create(ctx, createTestObject("Obj2", "Second.", world.Containment{LocationID: &room.ID})))).To(Succeed())
		...
		It("returns empty slice for empty location", func() {
			...
			Expect(delErr(env.Locations.Create(ctx, emptyRoom))).To(Succeed())
	...
	Describe("ListHeldBy", func() {
		It("returns objects held by character", func() {
			Expect(delErr(env.Objects.Create(ctx, createTestObject("Held1", "First held.", world.Containment{CharacterID: &charID})))).To(Succeed())
			Expect(delErr(env.Objects.Create(ctx, createTestObject("Held2", "Second held.", world.Containment{CharacterID: &charID})))).To(Succeed())
	...
	Describe("ListContainedIn", func() {
		It("returns objects inside container", func() {
			...
			Expect(delErr(env.Objects.Create(ctx, createTestObject("Item1", "First item.", world.Containment{ObjectID: &container.ID})))).To(Succeed())
			Expect(delErr(env.Objects.Create(ctx, createTestObject("Item2", "Second item.", world.Containment{ObjectID: &container.ID})))).To(Succeed())
