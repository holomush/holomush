...
| Gap | Detail |
| --- | --- |
| **Game display name is server-side-only** → **[#4905](https://github.com/holomush/holomush/issues/4905)** | `SettingConfig.DisplayName` is **required** on setting plugins but reaches no web surface, while an **optional** `landing.hero.metadata.title` content key (via `ContentService.ListContent`, `routes/+page.ts`) is what actually renders — falling back to the platform brand. Any player-facing game identity — title tag, OG card, welcome line, a "back" target — needs this settled first. **Correction:** 010's claim that *"the only `HoloMUSH` strings under `web/src/` are SPDX copyright headers"* was **false** — three real render sites exist (`TopBar.svelte:66` fine, `routes/+page.svelte:34` fallback, `terminal/+page.svelte:689` bare `<h1>`). |
