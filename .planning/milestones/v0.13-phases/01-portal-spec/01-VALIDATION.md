...
| PORTAL-10 (§12 six rules stated 1–6) | T1 | 01-05 | Six rules are stated and numbered under §12.1 | doc-assertion | `awk '/^### 12\.1 /,/^### 12\.2 /' 01-SPEC.md \| rg -c '^[0-9]\. '` → **6** | `01-SPEC.md:2805-2941` | ✅ COVERED (count only) |
| PORTAL-10 (§12 each rule carries a non-vacuity clause) | T1 | 01-05 | Every one of the six rules states what would make it vacuous | — | none — `MUST NOT` appears 5× across 6 rules, so a count cannot stand in for the claim | `01-SPEC.md:2805-2941` | MANUAL-BY-DESIGN |
