...
| 4 | `expected_version` on every mutation; role mutation an explicit exclusion; sorting/filtering answered with a verdict | ✓ VERIFIED (gap 2 closed) | §9.4 fixes `expected_version` as an `int32` scalar per guarded request message, rejects absent-or-zero with `CHARACTER_VERSION_REQUIRED`, and **normatively excludes `CreateCharacter`** across five mutually consistent references — four stating the exclusion directly (`:2018`, `:2059-2061`, `:2098-2103`, `:2217`, the four `01-UAT.md:105` counts) plus §9.3's preamble at `:2006-2013`, which scopes the rule to mutations targeting an existing row rather than restating the exclusion — with the `UNIQUE`-index substitute named. The rule is now implementable as written. §10.8 excludes role mutation alongside impersonation, break-glass identifiers and a raw DB console, each with a reason. §11.1 answers PORTAL-09 "**No**" with three ordered reasons and §11.3 bounds the permitted surface to six intrinsic/joined columns, none of them profile-bearing. |
...
- `01-UAT.md` was **untracked in git** when this report was written; it is now committed alongside this report, so the human-verification evidence for this phase is version-controlled rather than living only in the working tree. Resolved — retained here because the provenance note above refers to the state at verification time.
