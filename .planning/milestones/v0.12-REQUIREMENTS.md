# Requirements Archive: HoloMUSH — Milestone v0.12 (Foundation Hardening)

**Status:** ✅ SHIPPED 2026-07-28 · **Outcome:** 15 of 19 satisfied, 4 deferred with tracking

> Archived at milestone close. Requirement outcomes and their evidence are recorded in the
> `## Outcomes` table below; the full audit is `v0.12-MILESTONE-AUDIT.md`.
...
- [x] **OPS-01**: The public gateway caps request-body size (`connect.WithReadMaxBytes`) and sets a read timeout, so an unauthenticated client cannot OOM the gateway with an unbounded body — closes F2 (#4785). **Ships first as a `/gsd-quick` fix (pre-Phase 4)** — a live DoS one-liner, too small for the full phase loop.
...
Legend: `[x]` satisfied · `[~]` partially satisfied, deliberately deferred with tracking

| Requirement | Phase | Final status | Evidence / outcome |
| --- | --- | --- | --- |
| MODEL-01 | 4 | **Satisfied** | ADR `holomush-i4784` Accepted; decided at a blocking human checkpoint; names the Phase-5 mechanism |
| OPS-05 | 4 | **Satisfied** | Two-replica resilience harness, 12 specs; M12 last-write-wins empirically reproduced |
| MODEL-02 | 5 | **Satisfied** | Replay-derived-world-state claims removed; guard test `test/meta/world_model_doc_claim_test.go` |
| MODEL-03 | 5 | **Satisfied** | Version-predicated CAS in all four world repos; `WORLD_CONCURRENT_EDIT`; INV-WORLD-2 bound |
| MODEL-04 | 5 | **Satisfied** | Transactional outbox + leased ordered relay; post-commit emit path deleted; INV-WORLD-1/3 bound |
| OPS-02 | 6 | **Satisfied** | `events_audit` partitioned + RetentionWorker; owned by the AuditProjection subsystem |
| OPS-03 | 6 | **Satisfied** | nats-server ≥ 2.14.3; govulncheck + osv-scanner as the required `Vuln` check |
| OPS-04 | 6 | **Satisfied** | `game_id` bridge fixed; tautological coverage test replaced |
| QUAL-01 | 6 | **Satisfied** | Coverage docs reconciled to reality + `.codecov.yml` rising-floor ratchet. **Caveat:** no *enforced* gate exists — see below |
| ARCH-03 | 7 | **Satisfied** | Two-sweep Prepare/Activate across 17 subsystems; zero `Start` outside the orchestrator |
| ARCH-04 | 7 | **Satisfied** | `core.Event` deleted; `eventbus.Event` sole representation |
| ARCH-05 | 7 | **Satisfied** | Gateway closure gate; INV-EVENTBUS-1 bound; verified by `go list -deps` |
| ARCH-01 | 8 | **Satisfied** | `CoreServer` 1891→657 lines, 4 handlers; mutation-proven regrowth ratchet |
| ARCH-02 | 8 | **Satisfied** | `plugin/manager` 1876→702 lines, 3 units; whole-system census green |
| QUAL-04 | 9 | **Satisfied** | Verified by execution: 42/42 session-lifecycle specs, 0 skipped, exit 0; 48-row matrix + bijection meta-test |
| **OPS-01** | quick fix | **Satisfied (adjusted)** | Shipped **pre-Phase-4 as a `/gsd-quick` fix, outside the phase loop**, so no phase VERIFICATION attests it. Marked complete on direct code evidence: `connect.WithReadMaxBytes` at `internal/web/server.go:85`, `ReadHeaderTimeout` `:147`, `ReadTimeout` `:154`. The audit classified it `orphaned` for exactly this reason — the capability shipped, the attestation chain did not. |
| **QUAL-02** | 9 | **Deferred** | Backfill real and measured; the *gate* half unmet. `internal/tls` 88.12% (floor met), `cmd/holomush` 70.09% (9.91 short), project 79.11% (0.89 short). Ruling at `09-19-SUMMARY.md:223-233`. → #4861, #4875, #4876 |
| **QUAL-03** | 9 | **Deferred** | Zero-assertion sweep ran and was clean; the honest scope is "a statement about one predicate, not a clean bill of health" (`09-09-SUMMARY.md:213-224`). → #4860 |
| **QUAL-05** | 9 | **Deferred** | 4 of 5 arch-review Mediums landed in-tree; DEK read-cache deferred (#4792); the de-slop/humanization half deliberately not started (`09-02-SUMMARY.md:22,181`) |

## Requirement-level caveat worth carrying into v0.13

**QUAL-01 is satisfied as written but the ambition behind it is not met.** The requirement asked that the
documented bar and the enforced bar agree — and they now do, because the documentation was corrected
downward to match a bar that is *not enforced at all*. Protect-main ruleset `11923801` requires no codecov
context; `codecov/project` posts on no ref (#4875), and requiring `codecov/patch` would deadlock docs-only
PRs (#4876). Phase 9's success criterion 1 promised "the coverage gate passes repo-wide"; there is no gate.

Nothing about coverage currently blocks a merge. v0.13 should not inherit the assumption that it does.
