...
- [x] **PORTAL-01**: The SPEC defines the **audience matrix** (public / owner / admin) and a distinct
...
- [x] **PORTAL-02**: The SPEC contains a **read-surface inventory** enumerating every character-returning
...
- [x] **PORTAL-03**: The SPEC contains a **name-capture surface inventory**, giving each site a
...
- [x] **PORTAL-04**: The SPEC defines the character **lifecycle states** with `retire`, `idle-out`, and
      `purge` as three distinct operations, and states that **retire MUST NOT release the name**.

- [x] **PORTAL-05**: The SPEC defines the profile/media data model as `entity_properties` rows
...
- [x] **PORTAL-06**: The SPEC defines the full new RPC surface with **`expected_version` on every mutation
      request message** (adding it later is a wire-compat change to every caller).

- [x] **PORTAL-07**: The SPEC states the name-normalization policy for character names and for player
      usernames as **two separate policies** (see IDENT-06/IDENT-07).

- [x] **PORTAL-08**: The SPEC records an **explicit exclusion**: role mutation is not part of character
      administration in this milestone. An omission is not an exclusion.

- [x] **PORTAL-09**: The SPEC answers whether **any v0.13 surface sorts or filters on a profile field**
...
- [x] **PORTAL-10**: The SPEC mandates the **verification-integrity rules** below, and every phase plan
