...
# scripts/pyproject.toml declares requires-python = ">=3.12". Check the
# interpreter version BEFORE importing 3.12-only symbols so older versions
# get a clear error instead of an obscure ImportError.
import sys

if sys.version_info < (3, 12):  # noqa: UP036 — intentional runtime gate; see comment above
    sys.stderr.write(
        f"bootstrap_seed_secrets.py requires Python 3.12+ "
        f"(detected {sys.version_info.major}.{sys.version_info.minor}). "
        f"Install a newer interpreter or run via `uv run python "
        f"scripts/bootstrap_seed_secrets.py` (uv manages the toolchain).\n"
    )
    sys.exit(1)

import argparse  # noqa: E402 — imports below require Python 3.12
...
def _write_secret(name: str, value: str, repo: str) -> bool:
    ...
    # Passing `--body -` sets the literal string "-" as the secret.
    # (CRITICAL bug caught by CodeRabbit review.)
    rc, _, err = _run_cmd(
