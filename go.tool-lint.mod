// Isolated tool-dependency module for linters (Go 1.24+ `go tool`).
//
// Separate from go.tool.mod: golangci-lint's analyzer closure and
// actionlint's YAML parser conflict with the charmbracelet/go-yaml versions
// `task` pulls in, so a single shared tools module fails to compile. Lint
