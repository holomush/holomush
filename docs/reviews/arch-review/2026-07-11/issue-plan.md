...
| I7 (F3) | `bug: audit-DLQ replay CLI can't recover external-NATS deployments (game_id split)` | `bug`,`priority::high`,`review-finding` | high | `findings/d6-reliability.md` (H1), `verification/skeptic-d6-dlq.md`. Server DLQ subject uses `core.game_id`/ULID (`core.go:300-304,567`); the CLI leaves `event_bus.game_id` empty (`cmd_audit.go:143-149`), so `ReplayDLQ` defaults the DLQ subject to `internal.main.audit.dlq` — the fix must resolve the server's *persisted* game_id, not read the event-bus default. AC: resolve `game_id` in `runAuditDLQReplay` via `holomush_system_info` (its PG pool is already open) and pass it in, OR add `--game-id`; scope note: external-NATS only, no data loss. Pairs with I4. |
...
| I10 (F8) | `deps: bump nats-server to v2.14.3 (2 GHSA 2026-06-29; govulncheck-blind)` | `bug`,`priority::high`,`review-finding` | high | `findings/d9c-deps.md` (H1) + `evidence/deps/`. Filed as #4790 (the standalone tracking issue) regardless of Renovate; AC: verify + merge the Renovate PR to v2.14.3 (or bump directly if absent), and add a vuln-scan CI gate (govulncheck) — see I15. |
...
- **6 standalone High** (I1 architecture + I5–I8, I10) + **1 standalone Medium** (I9 typed-Move RPC) + **1 investigation** (I11)
