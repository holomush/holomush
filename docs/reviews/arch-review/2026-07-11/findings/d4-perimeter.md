...
- **Recommendation:** Default `SecureCookies` to `true` and require an explicit `--dev-insecure` (or `--allow-insecure-cookies`) opt-out for local plain-HTTP dev. Deriving `Secure` from `X-Forwarded-Proto` is safe **only** behind a trusted proxy that strips/overwrites any client-supplied value — otherwise a client can spoof the header and steer the security decision — so prefer an explicit deployment setting over trusting the header. Fail-safe defaults belong on the secure side.
- **Dedup:** none
