...
- **Recommendation:** Smallest fix: `dlqConfigForGame` is a pure `gameID string` helper, so it can't do the DB lookup itself — instead resolve the game ID in `runAuditDLQReplay`, which already opens a Postgres pool (`openAuditPool`, `cmd/holomush/cmd_audit.go:319`) before calling `ReplayDLQ`: call `PostgresEventStore.GetSystemInfo(ctx, "game_id")` (mirroring `InitGameID`, `internal/store/postgres.go:70,97-101`) when `event_bus.game_id` is unset, then pass the resolved value into `dlqConfigForGame`, so the CLI derives the same value the server did. Failing that, add an explicit `--game-id` flag to `dlq replay`/`list`/`show` and document `event_bus.game_id` in `configuration.md`. Either way, document the two-setting split in the external-NATS runbook.
- **Dedup:** none
