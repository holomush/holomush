...
| `INV-PRIVACY` | Stream history temporal floors, scope gating, guest-session bounds, reattach/Idle arrival-timestamp semantics, web profile read-surface disclosure shape (existence non-disclosure, minimum-identity floor) | Privacy-relevant gating on reads. Does NOT include: ABAC policy evaluation (→ INV-ACCESS), subscribe authorization (→ INV-EVENTBUS). |
...
| `INV-WORLD` | World-model write integrity: atomic state+feed emission, delta-parity of the affected-aggregates manifest, ordered gap-free feed publication, and the raw-world-write boundary. Registered by 05-12 (Phase 5, MODEL-04). ALSO covers the shape of the world.Service COMMAND API itself -- the typed caller argument every authorizing command takes in its second parameter slot, and the caller's execution context that checkAccess forwards to the ABAC request (INV-WORLD-8, AUTHZ-01, phase 02.1). | World-model correctness guarantees born from the MODEL-01 ADR (holomush-i4784, Option B: CRUD-canonical world state + optimistic-concurrency version guard + transactional outbox / ordered atomic feed), PLUS the world.Service command-API caller argument and checkAccess's forwarding of that caller's execution context (subject string, system marker, attribute channel) into the ABAC request -- world-owned surface that is not itself born from MODEL-01, added deliberately by phase 02.1 (AUTHZ-01) rather than filed under INV-ACCESS, because the guarantee is about the shape of world.Service's own API and not about how access control decides. Does NOT include: ABAC policy-evaluation semantics, the S1 system-subject double-gate's own behavior, or attribute-provider behavior, all of which remain INV-ACCESS; crypto payload encryption (→ INV-CRYPTO), plugin-owned scene storage (plugin_core_scenes.scene_participants is out of the world fence, D-05), migration timestamp discipline (→ INV-STORE). status: pending because internal/world/ carries pre-existing FOREIGN bare INV-N tokens from other design specs (the holomush-72ou per-property-ABAC INV-1/INV-2 in service.go / service_test.go) that the provenance residual-walk would misattribute to this scope; the four INV-WORLD-N entries are nonetheless binding: bound (born canonical — no legacy-token migration is pending for THIS scope). |
...
| `INV-SCENE-65` | SceneService mutating lifecycle/membership RPC handlers (EndScene, PauseScene, ResumeScene, UpdateScene, InviteToScene, KickFromScene, TransferOwnership, LeaveScene) MUST self-enforce ABAC by evaluating the per-verb action on scene:<id> with the subject taken from the dispatch-actor context, so authorization holds for every caller — the telnet command path and the web SceneAccessService facade alike — not only when the telnet command wrapper gates first. Publish handlers are excluded by INV-SCENE-33 (participant-only read path forbids the ABAC engine). | — | bound |
| `INV-SCENE-66` | A scene-focused connection's ambient conversational verbs (pose/say/ooc/emit, including the :/;/" sigil aliases) route to the focused scene's IC/OOC stream and never leak to the grid location; the leading sigil never reaches content and no-space/OOC-style semantics are preserved across the top-level focus redirect. | — | bound |
...
| `INV-PLUGIN-45` | The declaration gate that enforces least privilege MUST live at the broker/registry common path shared by both runtimes; per-runtime gating that could diverge is forbidden. | — | bound |
...
| `INV-ACCESS-12` | The set of RPCs whose response transitively contains a character-shaped message equals the set enumerated in the portal SPEC's read-surface inventory (§3) — no more and no fewer. Compared by SET EQUALITY on the fully-qualified proto method name (package.Service.Method), order-independent, never a substring match and never a Go handler identifier: set equality is what carries both halves, where a loop over a hand-written list carries neither. A character-returning RPC added without an inventory row fails the census RED rather than shipping. | — | pending |
...
| `INV-STORE-10` | The golang-migrate adopt seed inserts goose_db_version rows in ASCENDING version order. goose_db_version.id is GENERATED BY DEFAULT AS IDENTITY and goose's ListMigrations is ORDER BY id DESC, so insertion order — not version order — determines the order a later rollback walks migrations. A descending seed satisfies every row-count, set-equality, and min/max assertion while rolling back in the wrong order. | — | bound |
| `INV-STORE-11` | Every versioned .go file in internal/store/migrations/ registers itself with goose in init(), and package store blank-imports that directory so those init() functions run. //go:embed migrations/*.sql never embeds .go, so goose's own unregistered-Go guard cannot fire here — a migration that forgets its registration, or a dropped blank import, leaves the Go step SILENTLY absent from the chain with correct version counts, a consistent schema, and a green suite. | — | pending |
...
| ID | Summary | Legacy | Binding |
|----|---------|--------|---------|
| `INV-WORLD-1` | ATOMIC-FEED: a world state change and its one semantic outbox envelope commit or roll back ATOMICALLY in the SAME transaction — an envelope-less committed state change, or a committed envelope for a rolled-back state change, is impossible. Bound to the always-run state+envelope atomicity test (real world row + envelope: rollback→neither survives, commit→both survive, forced outbox failure after the state write→state rolls back). | `INV-WORLD-ATOMIC-FEED` | bound |
| `INV-WORLD-2` | DELTA-PARITY: the affected-aggregates manifest on the emitted envelope provably MATCHES the committed MutationDelta the guarded write returned — every affected aggregate's id, tombstone flag, and before/after versions equal the row's actual version transition, including DB-cascaded children (a location DELETE's cascaded exit tombstones) and a bidirectional exit's reverse aggregate. Manifest PRESENCE is insufficient — parity of the versions is the guarantee. | `INV-WORLD-DELTA-PARITY` | bound |
| `INV-WORLD-3` | FEED-ORDER: the single leased relay publishes strictly in feed_position order with Nats-Msg-Id = the event ULID, a poison envelope HALTS the feed at its position (rows after it are withheld), and an operator skip leaves NO missing feed_position — a skip marker is published at the skipped position so the wire has no gap. | `INV-WORLD-FEED-ORDER` | bound |
| `INV-WORLD-4` | WRITER-BOUNDARY: raw world-table mutation SQL (locations/exits/characters/objects/entity_properties/outbox/ world_feed_counter) is confined to internal/world/postgres — enforced by the AST SQL fence, which ALSO scans MIGRATION files (a migration mutating a world table without an envelope/epoch marker fails the fence — round-6 Codex MEDIUM: the Go-source fence cannot see SQL migrations, and 000001_baseline already seeds world rows). world.Service exposes no directly-callable writer repo (its fields are reader views — the compile fence), and ONLY composition/test packages may import internal/world/postgres (the import-graph allowlist). The write executor is the sole in-world writer-repo owner; there are exactly THREE sanctioned out-of-world writers, each emitting its envelope atomically: the character-genesis application service (05-15, CREATION), the character-reaping application service (05-16, D-06, DELETION — guest reaping + failed-guest cleanup emit one character tombstone per reaped character; previously an FK cascade with no tombstone), and the operator name-resolution command `holomush character name set` (02-12, RENAME — it runs in cmd/holomush, is in the import-graph allowlist, holds no characters SQL of its own, and writes through CharacterRepository.Rename, which 02-06 made emit its outbox envelope INSIDE its own transaction, so the CLI's write carries an envelope even though the command discards the returned delta). The guest-DELETION side is proven by the D-06 tombstone regression so the FK-cascade hole cannot regrow; the RENAME side is proven by the CLI integration spec, which observes the outbox row committed alongside the renamed row. The count was TWO until 02-12 shipped the third writer; it is amended deliberately rather than the atomicity clause being weakened, because what was false was the enumeration and not the guarantee. | `INV-WORLD-WRITER-BOUNDARY` | bound |
| `INV-WORLD-5` | LIFECYCLE-EXHAUSTIVE: every read of characters.status is an exhaustive switch over the closed vocabulary ('active','retired','idle') whose default arm DENIES, and a character in the third, otherwise-unreachable 'idle' state is excluded from character selection. v0.13 ships the 'idle' value with no transition into it, so the binding test MUST construct a character directly in 'idle' (bypassing the absent transition) and assert the exclusion — an unreachable enum value guarded by a non-exhaustive check is the fail-open shape research CONFLICT 4 rejected. | — | bound |
| `INV-WORLD-6` | RETIRE-PRESERVES-NAME: retiring a character leaves its row and its name reservation intact; a character name becomes claimable again only through a sanctioned tombstone-emitting hard delete, and there are exactly TWO such paths — world.Service.DeleteCharacter (the in-world authorized delete) and auth.CharacterReapingService (guest reaping, the second sanctioned out-of-world writer under INV-WORLD-4). Both are asserted; an enumeration proven on one path while claiming two is a fabricated guarantee no meta-test can detect. Retire MUST NOT release the name — a freed name claimed by a new character inherits the identity of every display name already denormalized into immutable event payloads and published scene archives, which no later write can reach. | — | bound |
| `INV-WORLD-7` | GUARDED-CHARACTER-MUTATION: every v0.13 character mutation RPC THAT TARGETS AN EXISTING character row carries expected_version as an int32 scalar on its own request message (matching characters.version, migration 000049), REJECTS an absent or zero value instead of writing unguarded (the repo layer's Version==0 unversioned-write affordance MUST NOT be reachable from a web mutation boundary), and rejects a stale value with the typed WORLD_CONCURRENT_EDIT signal so exactly ONE of two concurrent mutations sharing a version succeeds and the other is refused without auto-retry. CreateCharacter is EXCLUDED from the version guard because it creates the row the guard would protect: it carries no expected_version, and concurrent creates are decided by the UNIQUE index on the stored normalized name (CHARACTER_NAME_TAKEN) instead. The same-transaction obligation is NOT scoped that way -- EVERY v0.13 character mutation RPC, creation included, commits its state change together with its one outbox envelope in the SAME transaction (INV-WORLD-1) so a committed character mutation without its envelope is impossible. | — | pending |
| `INV-WORLD-8` | CALLER-ARGUMENT: every public world.Service COMMAND -- an exported *Service method whose first parameter is a context.Context -- that performs an authorization check (its body calls the receiver's checkAccess) declares the opaque world.Caller value in its second parameter slot rather than a bare subject string, and checkAccess forwards that caller's attribute channel to the ABAC request (types.NewAccessRequest's attributes argument) rather than a hardcoded nil, so a caller-carried attribute surfaces in the policy DSL as action.<key> and SystemCaller() satisfies BOTH halves of the S1 double-gate without the call site stamping access.WithSystemSubject of its own. NO public world.Service command -- authorizing or not -- declares a bare string in that second slot. world.Service.UpdateCharacterPreferences is the SINGLE deliberate exception to the caller-argument clause: it takes no subject at all (its second parameter is the character ULID) and deliberately runs no checkAccess, because it is the settings-subsystem persistence primitive whose authorization decision is made at the settings command layer and whose predecessor raw-store path had no ABAC -- gating it here would be an out-of-scope behavior change (D-05, per internal/world/service.go:862 and that method's own doc comment). The scoping to COMMANDS is likewise deliberate and is mirrored word-for-word by the census predicate: world.Service.SetMovementHook (internal/world/service.go:160) is an exported lifecycle setter with no context.Context and no authorization check, so a summary phrased over every exported world.Service METHOD would claim more than the binding proves. The census is structural over go/ast, not a name list, so it follows a command that gains or loses its authorization check and catches a future command written with a bare subjectID string -- which the compiler would accept (AUTHZ-01, phase 02.1). | — | bound |
