...
| `INV-WORLD-4` | WRITER-BOUNDARY: raw world-table mutation SQL (locations/exits/characters/objects/entity_properties/outbox/ world_feed_counter) is confined to internal/world/postgres — enforced by the AST SQL fence, which ALSO scans MIGRATION files (a migration mutating a world table without an envelope/epoch marker fails the fence — round-6 Codex MEDIUM: the Go-source fence cannot see SQL migrations, and 000001_baseline already seeds world rows). world.Service exposes no directly-callable writer repo (its fields are reader views — the compile fence), and ONLY composition/test packages may import internal/world/postgres (the import-graph allowlist). The write executor is the sole in-world writer-repo owner; there are exactly TWO sanctioned out-of-world writers, each emitting its envelope atomically: the character-genesis application service (05-15, CREATION) and the character-reaping application service (05-16, D-06, DELETION — guest reaping + failed-guest cleanup emit one character tombstone per reaped character; previously an FK cascade with no tombstone). The guest-DELETION side is proven by the D-06 tombstone regression so the FK-cascade hole cannot regrow. | `INV-WORLD-WRITER-BOUNDARY` | bound |
