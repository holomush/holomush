...
Other concurrent sessions for the same character maintain independent floors; one session's reattach does NOT affect another session's visibility.
...
**Multi-session worked example.** Web session A and telnet session B both belong to character X. A attaches at T=0. B attaches at T=10. A drops at T=20 (status=Detached, expires_at=T=20+1800s); B remains attached. Events emitted at T=25 are visible to B (continuously attached). A reattaches at T=30 (within TTL). After reattach: A's `LocationArrivedAt = 0` (unchanged); A's `QueryStreamHistory` returns events with `timestamp >= 0`, including the T=25 events emitted during A's transport disconnect. The durable JetStream consumer for A's session held those events for redelivery on reattach — they are part of A's session's continuous existence.

If instead A's TTL had expired before A reconnected (e.g., A drops at T=20, reaper sweeps at T=20+1800s, A reconnects at T=2000), `SelectCharacter` would find no session row and create a fresh one with `LocationArrivedAt = 2000`. The fresh session's floor naturally excludes events from the prior lifetime.
...
- **Integration:** `TestPrivacy_MultiSessionContinuity` — character X has web+telnet sessions. Web first attaches at T0, telnet first attaches at T0' (each session has independent `LocationArrivedAt`). Web's transport drops at T1, telnet stays. Events emitted at T2. Web's `Subscribe.ReattachCAS` at T3 (within TTL — same row). Telnet's history query at T4 returns events from telnet's `LocationArrivedAt` (T0') forward. Web's history query at T4 returns events from Web's `LocationArrivedAt` (T0) forward — INCLUDING the T2 events emitted during Web's transport disconnect (the session row was held open; durable consumer received those events).
