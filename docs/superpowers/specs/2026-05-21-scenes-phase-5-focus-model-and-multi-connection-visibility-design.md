...
| **INV-P5-12** | Reconnect restoration vs concurrent `LeaveFocus` serializes via the single Store-lock acquisition: either the leave commits first (restoration's mutator sees no membership → grid fallback) OR the restoration commits first (the leave's subsequent scene_leave_ic reaches the now-subscribed connection on the wire). No corruption; no torn state. | `internal/grpc/focus/restore_connection_focus_test.go::TestRestoreConnectionFocusSerializesAgainstAConcurrentSceneLeave` (renamed from `TestReconnect_VsConcurrentLeave_Serializes` by the phase-09 ACE sweep) |
...
- `test/integration/scenes/reconnect_focus_restoration_test.go` — INV-P5-5 reconnect with PresentingFocus + revoked-membership fallback + TestRestoreConnectionFocusSerializesAgainstAConcurrentSceneLeave (INV-P5-12) + TestReconnect_AfterSceneGrid_RestoresPriorScene (INV-P5-13 end-to-end)
