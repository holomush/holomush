...
| D7 | **Unread badges = live notifications + snapshot catch-up.** Substrate-side fanout: when an event lands on a scene stream, the subscription router sends a lightweight `scene_activity` notification to connections of sessions whose `FocusMemberships` include that scene and which are not currently focused on it. `WebListMyScenes` returns last-activity/pose-count snapshots for initial sync and reconnect catch-up. The client de-duplicates (focused scene's own events are not also counted via notifications). Observers receive badges like any member (D6). | User decision: "ConnectRPC is live for a reason — 2 with a 1-based catch-up." `FocusMemberships` is server-authoritative and ⊆ `scene_participants` (Phase 5 D4), so participants-only fanout holds by construction (INV-SCENE-62). No plugin manifest/emit changes needed. |
| D8 | **The log viewer consumes structured data.** A new participant-gated `SceneService.ReadSceneLog` RPC returns typed, paginated entries (kind: pose/say/ooc/system, actor, body, timestamp, content-warnings). The client renders entries as Svelte components. No markdown parsing in the viewer; `marked`/`dompurify` are NOT used. | The log is data (`plugin_core_scenes.scene_log`); markdown is one *export rendering* of it. Parsing exported markdown back into a UI would discard structure. |
...
| Layer | Change |
|---|---|
| `api/proto/holomush/scene/v1/scene.proto` | **Add** `ReadSceneLog` (structured, paginated, participant-gated; spectate-extended per D6) and `ListCharacterScenes` (D12). Full proto doc comments grounded in handlers (repo rule). |
| `api/proto/holomush/web/v1/web.proto` | **Add** `WebListScenes`, `WebGetScene`, `WebListMyScenes`, `WebReadSceneLog`, `WebExportScene`, `WebGetPublicSceneArchive`, `WebDownloadPublicSceneArchive`, `WebWatchScene` (observer auto-join, D6), and `WebSetSceneFocus` (focus/clear on a workspace connection — membership-gated as built). |
...
**Read an ended/published scene.** `/scenes/[id]` → `WebGetScene` + paginated `WebReadSceneLog` (participant or published-visibility rules) → static typed log; export buttons.
...
|---|---|---|
| V1 | A player session supports multiple concurrent child character sessions (game-session fanout exists in `auth_service.go`; confirm store + liveness semantics for N alts). | D3 needs a session-model extension instead of reuse. |
| V2 | Establishing a workspace-only character session does not announce grid arrival / set grid presence (terminology: grid-present = terminal/telnet conn). If it does, a quiet-session variant is required. | Scene-only players would "appear" on the grid. |
...
| V4 | `scene pose`/`say`/`ooc` addressing from a workspace connection (focused-scene-relative vs explicit `#id`) and behavior when the alt has multiple focused connections. | Composer writes land in the wrong scene. |
| V5 | Transport shape for `scene_activity` (control-frame variant vs lightweight event on the existing Subscribe stream) and its TS client handling. The implementation plan MUST pin this contract as an explicit task before any badge work starts. | Badge channel design rework. |
| V6 | `ReadSceneLog` pagination contract vs `scene_log` schema (cursor vs offset; CW annotations per entry). | Viewer pagination rework. |
