...
| §11.3 step 5 (line 2185) | Rewrite from "Decide on TOTP enrollment for wizard accounts. Required for `Rekey` and `AdminReadStream` once enabled in config." to "Verify TOTP enrolled for admin accounts who hold the `crypto.operator` capability. Required for `Rekey` and `AdminReadStream`." Combines Decision 2's `s/wizard/admin/` with Decision 3's hard-required-TOTP semantic. (Drift fix: an earlier draft of this table cell misattributed the target to §12; the substring lives in §11.3.) |
| §4.6 line 833            | `s/<wizard player_id>/<admin player_id>/` in the rekey audit-event example actor metadata (`actor: {kind: operator, os_user: <uid>, player_id: <admin player_id>}`). (Drift fix: earlier draft of this table omitted this row; Decision 2's `s/wizard/admin/` should reach this line.) |
