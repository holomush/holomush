...
| `dekmaterialnoslog` | `log/slog.Info`, `Debug`, `Warn`, `Error`, `InfoContext`, `DebugContext`, `WarnContext`, `ErrorContext`, `Log`, `LogAttrs`, `Any`, `Group`, plus the `*Logger` method equivalents (`(*slog.Logger).Info`, `.InfoContext`, `.LogAttrs` etc.; `Any` and `Group` are package-level helpers, not methods) |
