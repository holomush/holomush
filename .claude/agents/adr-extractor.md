...
4. **Not already captured** in `docs/adr/` — you MUST grep / probe the
   directory and run `gh issue list -R holomush/holomush --label decision
   --state all` before proposing a new candidate. If a related ADR exists,
   propose `supersedes` rather than "new."
  ...
  "candidates": [
    {
      ...
      "supersedes": null | "<github-issue-number>"
    }
  ],
  "dropped": [
