...
- **`grpc.Client` wrappers swallow error codes before callers can classify them (RESOLVED rsoe6.11).** A bare-oops return from a `Client.X` method wraps all errors as `RPC_FAILED`, masking wire codes needed for terminal-vs-transient classification. Fix pattern: add a `TranslateXErr` exported helper that calls `status.FromError` and maps specific `codes.*` values to oops codes; call it in the Client wrapper. Mock-based tests bypass the wrapper silently — require a test that injects `status.Error(codes.Unauthenticated, ...)` through the REAL wrapper. For server-streaming RPCs, grpc-go returns (stream, nil) and defers the handler error to the first Recv — the gateway recv goroutine must call TranslateXErr on the recv error and stash it before closing the event channel (happens-before via close). Encountered: holomush-rsoe6.11 (2026-05-31) — RESOLVED round2.
