...
## ADR-capture / shell-hook reflexes (HoloMUSH 2026-05-14)

- **`jj mv` does NOT exist.** Verified via `jj --help | grep -iE 'mv|rename|move'` — only `absorb / next / rebase / squash` returned. jj has no rename subcommand by design — the snapshotter auto-detects renames from working-copy file moves. Plans (and specs) that say "use `jj mv` to preserve history" are wrong twice over: the property (`jj log --follow` continuity) is preserved by jj's auto-rename detection WITHOUT any CLI subcommand. Migration scripts should use `Path.rename()` / `shutil.move` / `mv`; jj will track. Subprocess calls to `jj mv` exit non-zero at runtime with `error: unrecognized subcommand 'mv'`.
- **`bd create` stdout format**: `✓ Created issue: holomush-XXXX — <title>` (unicode bullet + em-dash; verified live 2026-05-14). Regex `r"Created issue: (\S+)"` works against this; ID is the first non-space sequence. Plans should self-test the regex at module import.
- **`bd dep add <from> <to> --type <reltype>`** is correct positional form (verified `bd dep add --help`). `--type supersedes` works. `bd close -r/--reason "..."` and `bd delete -f` are both valid.
- **Bash `$(...)` strips trailing newlines.** A hook computing SHA via `stripped="$(awk ... <file>)"; printf '%s' "$stripped" | sha256sum` hashes content WITHOUT the final `\n`. If a paired writer (skill, migration) computes SHA over `normalized_content` that ends with `\n`, the two values cannot agree. Plans claiming "the hook and the writer compute the same SHA" must explicitly nail down whether input bytes include the trailing newline, AND the test harness must exercise the writer SHA via a different code path than the hook SHA — a `spec_sha()` helper that reuses the hook's bash pipeline produces fake agreement that breaks in production. Verified by reproducing in `/tmp/probe-marker.txt`: hook-side `683376e290829b48` vs file-content-side `2751a3a2f303ad21`.
- **Existing-repo `**Status:** Superseded by …` format heterogeneity.** HoloMUSH's `0007-command-security-model.md` uses `**Status:** Superseded by [ADR 0014](0014-...md)` (SPACE between `ADR` and digits, MARKDOWN-LINK form). README index uses `Superseded by 0014` (no `ADR` prefix). A regex `r"Superseded by ADR-?(\d+)"` matches NEITHER. Migration parsers MUST use a liberal alternation like `r"Superseded by\s+(?:\[)?ADR[\s-]?0*(\d+)"`. ALWAYS verify supersession regexes against the real repo files before approving.
- **Taskfile `lint:` task uses `cmds: [- task: lint:xxx, ...]`, NOT `deps:`.** Plans saying "add `lint:adr` to `lint`'s `deps:` list" cannot be applied — the field doesn't exist on this task. The correct edit is "append `- task: lint:adr` to `cmds:` list." Always grep the current Taskfile and show the BEFORE/AFTER block in the plan.
- **Dogfood-step file-count conflict with strict-equality invariants.** When a plan ends with "Task N: run the skill on the spec; capture ADRs MAY be 0–5" AND the doctor asserts "exactly 17 real ADR files + exactly 17 stubs + exactly 1 README = exactly 35 total," accepting any candidate in the dogfood step breaks the doctor. Plans MUST either loosen the invariant to `>= 17` / `>= 35`, or instruct the dogfood step to skip every candidate (which makes the proof weaker). Watch for this conflict when a strict-equality count is tagged `doctor` AND a dogfood step is in the same plan.
- **`bd dolt commit` before `verify_post_migration` is a recovery hole.** When a migration script commits to dolt BEFORE running its inline asserts, an assert failure followed by `jj abandon` rolls back the working copy but the bd records are already in dolt. Plans claiming "single jj change; recovery is `jj abandon` + rerun" must defer ALL state-changing operations (including `bd dolt commit`) until AFTER the verify, or the recovery is incomplete and leaves orphaned bd records.
