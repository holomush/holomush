...
| — | **The game's display name is server-side-only.** `SettingConfig.DisplayName` (`internal/plugin/manifest.go:211`) is **required** yet reaches **no** web surface, while an *optional* `landing.hero.metadata.title` content key is what actually renders — falling back to the platform brand | **Filed**, [#4905](https://github.com/holomush/holomush/issues/4905) — blocks any player-facing game identity (title tag, OG card, welcome line, "back" target) |
